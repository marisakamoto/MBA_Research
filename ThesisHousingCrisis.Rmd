...
p <-population %>% 
ggplot(aes(x = Year, y = VALUE)) + geom_line()  + ylab("Population")

plotly::ggplotly(p)


t_population <- population %>% ungroup() %>% select(Year, VALUE)
...
t_houseRegistered <- by_group %>% ungroup() %>% filter(County =='All Counties')  %>% select(Year, sum)
